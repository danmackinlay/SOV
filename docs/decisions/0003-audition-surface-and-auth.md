...
## Why Open WebUI not LibreChat or similar
...
- LibreChat is more featureful but heavier to deploy and configure; we don't need its multi-provider support.
...
- Open WebUI's "single-user mode" config is documented and the same across phases.
- Scripts assume RunPod's proxy URL by default; phase 1 may add a Cloudflare Tunnel codepath for Lambda.

## Re-evaluate when

- We move toward actual cooperative formation and recurring members. At that point, ADR 0006-or-whatever introduces real auth — likely an OAuth-fronted nginx + per-user keys + capped quotas. That is a phase-3 problem, not earlier.
