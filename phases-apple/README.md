...
- **LiteLLM (phase 1 onward) pinned by digest, never internet-exposed.** Pin a known-good version ≥1.83.7 (CVE-2026-42208 pre-auth SQL injection fixed there; a March 2026 PyPI supply-chain attack pushed malicious 1.82.7/1.82.8). On a laptop the blast radius is small but the discipline is identical to the cloud-track rule. **Always launch with `--host 127.0.0.1`** — LiteLLM defaults to `0.0.0.0`, which binds all interfaces and exposes your proxy (and the model behind it) to anyone on the same wifi. The CLI flag is the only way to bind loopback-only; there's no YAML equivalent.
