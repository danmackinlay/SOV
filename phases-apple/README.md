...
- **LiteLLM (phase 1 onward) pinned by digest, never internet-exposed.** Pin a known-good version ≥1.83.7 (CVE-2026-42208 pre-auth SQL injection fixed there; a March 2026 PyPI supply-chain attack pushed malicious 1.82.7/1.82.8). On a laptop the blast radius is small but the discipline is identical to the cloud-track rule. **Bind loopback-only** — LiteLLM defaults to `--host 0.0.0.0` (exposes the proxy to anyone on your wifi). Two ways to fix, both belt-and-braces: (a) the committed [`phases-apple/.envrc`](.envrc) sets `HOST=127.0.0.1` which LiteLLM reads via its `envvar="HOST"` Click option, so cwd anywhere under `phases-apple/` is automatically safe — at repo-root cwd, mirror the export in your `.envrc.local`; (b) pass `--host 127.0.0.1` on the CLI as belt-and-braces in the canonical command. No YAML equivalent exists.
